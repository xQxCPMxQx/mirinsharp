...
namespace Warwick
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            {
                menuAuto.AddItem(new MenuItem("Auto.Q.Title", "Q Settings"));
                {
                    menuAuto.AddItem(
                        new MenuItem("Auto.Q.ToEnemy", Tab + "Auto Q to Enemy").SetValue(
                            new StringList(new[] {"Off", "Everytime", "Just High Priority Enemies"}, 1)));
                    menuAuto.AddItem(
                        new MenuItem("Auto.Q.UseQHp", Tab + "Keep-Up My Heal Min.:").SetValue(new Slider(70, 1)));
                    menuAuto.AddItem(
                        new MenuItem("Auto.Q.UseQHpMana", Tab + Tab + "Min. Mana Per.:").SetValue(new Slider(20, 1)));
                }
        ...
        private static bool GetPriorityAllies(string championName)
        {
            if (
                new string[]
                {
                    "Olaf", "Renekton", "Shyvana", "Sion", "Skarner", "Thresh", "Volibear", "MonkeyKing", "Yorick",
                    "Aatrox", "Darius", "Diana", "Ekko", "Evelynn", "Fiora", "Fizz", "Gangplank", "Gragas", "Irelia",
                    "Jax", "Jayce", "Kayle", "Kha'Zix", "Lee Sin", "Nocturne", "Nidalee", "Pantheon", "Poppy", "RekSai",
        ...
        private static void Game_OnUpdate(EventArgs args)
        {
            ...
            {
                if (Config.Item("Auto.Q.UseQHp").GetValue<Slider>().Value > Player.HealthPercent)
                {
                    ...
                    List<Obj_AI_Base> enemyObjects =
                        ((from obj in
                            ...
                            (from obj in HeroManager.Enemies
                                where obj.IsValidTarget(Q.Range) && !obj.IsDead
                                select obj)
                            .Union
                            (from obj in
        ...
        private static void Combo()
        {
            ...
            if (R.IsReady())
            {
                ...
                switch (useR)
                {
                    ...
                    case 2:
                    {
                        if (tR.IsValidTarget(R.Range) && tR.Health < GetComboDamage(tR))
                            R.Cast(tR);
                        break;
                    }
                    case 3:
                    {
                        if (tR.IsValidTarget(R.Range) &&
                            (tR.Health < GetComboDamage(tR) || tR.CountAlliesInRange(800) >= 2 ||
