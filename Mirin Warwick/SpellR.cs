...
namespace Warwick
{
    internal class SpellR
    {
        ...
        private static void Load()
        {
                ...
                foreach (var enemy in HeroManager.Enemies)
                {
                    Program.rMenu.AddItem(
                        new MenuItem("Selected" + enemy.ChampionName, MenuTab + enemy.ChampionName).SetValue(
                            new StringList(new[] {"Don't Use", "Low Priority", "Medium Priority", ">> High Priority <<"}, GetPriority(enemy.ChampionName))));
                }
            }
            ...
            {
                Program.rMenu.AddItem(
                    new MenuItem("R.AutoPriority Focus", MenuTab + "Auto Arrange priorities").SetShared().SetValue(false))
            ...
            {
                Program.rMenu.AddItem(new MenuItem("Draw.Search", MenuTab + "Show Search Range").SetValue(new Circle(true, System.Drawing.Color.GreenYellow)));
                Program.rMenu.AddItem(new MenuItem("Draw.Status", MenuTab + "Show Targeting Status").SetValue(true));
                Program.rMenu.AddItem(new MenuItem("Draw.Status.Show", MenuTab + "Show This:").SetValue(new StringList(new[] { "All", "Show Only High Priority Target Status" })));
            }
        ...
        private static void LoadEnemyPriorityData()
        {
            foreach (var enemy in HeroManager.Enemies)
            {
                Program.rMenu.Item("Selected" + enemy.ChampionName)
                    .SetValue(new StringList(new[] {"Don't Use", "Low Priority", "Medium Priority", ">> High Priority <<"}, GetPriority(enemy.ChampionName)));
            }
        }
        ...
        private static void Game_OnWndProc(WndEventArgs args)
        {
            ...
            if (Program.rMenu.Item("R.Click").GetValue<bool>())
            {
                        ...
                        .FirstOrDefault();
                {
                    ...
                    {
                        var vSelected = Program.rMenu.Item("Selected" + selectedTarget.ChampionName).GetValue<StringList>().SelectedIndex;

                        var i = vSelected == 3 ? 0 : vSelected + 1;

                        Program.rMenu.Item("Selected" + selectedTarget.ChampionName).SetValue(new StringList(new[] { "Don't Use", "Low Priority", "Medium Priority", ">> High Priority <<" }, i));
                    }
                }
            }
        }
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
            ...
            if (Program.rMenu.Item("Draw.Status").GetValue<bool>())
            {
                foreach (var a in HeroManager.Enemies.Where(e => e.IsVisible && !e.IsDead && !e.IsZombie))
                {
                    ...
                    if (Program.rMenu.Item("Draw.Status.Show").GetValue<StringList>().SelectedIndex == 1 && vSelected != 3)
                        continue;
